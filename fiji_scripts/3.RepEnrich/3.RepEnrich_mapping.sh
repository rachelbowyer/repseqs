...
R2file_name=Min_"$num"_S"$num"_R2_paired_trimmomatic


file_name_out=Min_"$num"_S"$num"_trimmed_paired_trimmomatic

bowtie $index -p 16 -t -m 1 -S --chunkmbs 1000 \
--max $output_folder/"$file_name_out"_multimap.fastq \
-1 $input_folder/"$R1file_name".fastq \
-2 $input_folder/"$R2file_name".fastq \
